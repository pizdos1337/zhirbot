...
async def fat_help(ctx):
    embed = discord.Embed(title="🍔 **ЖИРБОТ - ПОМОЩЬ** 🍔", description="Все команды", color=0xffaa00)
...
async def cooldown_info(ctx):
    ...
    embed = discord.Embed(title=f"⏰ Кулдауны на сервере {ctx.guild.name}", description=f"Для {ctx.author.mention}", color=0x3498db)
    embed.add_field(name="!жир", value=f"Кулдаун: {actual_fat_cooldown*60:.0f} мин\nСтатус: {'✅ Доступна' if fat_can_use else f'⏳ {format_time(fat_remaining)}'}", inline=True)
    embed.add_field(name="!жиркейс", value=f"Кулдаун: {actual_case_cooldown:.1f} ч\nСтатус: {'✅ Доступен' if case_can_use else f'⏳ {format_time(case_remaining)}'}", inline=True)
    embed.add_field(name="Текущий вес", value=f"{data['current_number']}kg", inline=True)
    await ctx.send(embed=embed)
...
async def upgrade_command(ctx, choice: str = None):
    ...
    if choice is None:
        ...
        items_text = "\n".join([f"**{i+1}.** {item['emoji']} **{item['name']}** — {item['count']} шт — {item['price']} кг" for i, item in enumerate(available_items[:20])])
    ...
    upgrades_text = "\n".join([f"**{i+1}.** {upgrade['emoji']} **{upgrade['name']}** — {upgrade['chance']*100:.1f}% шанс" for i, upgrade in enumerate(possible_upgrades[:20])])
...
async def upgrade_kg_command(ctx, amount: int):
    ...
    upgrades_text = "\n".join([f"**{i+1}.** {upgrade['emoji']} **{upgrade['name']}** — {upgrade['chance']*100:.1f}% шанс (нужно: {upgrade['price']} кг)" for i, upgrade in enumerate(possible_upgrades[:20])])
    embed.add_field(name="📈 Возможные улучшения", value=upgrades_text, inline=False)
    embed.set_footer(text="Кг уже списаны!")
    await ctx.send(embed=embed)
