...
async def fat_command(ctx):
    ...
    change, was_minus, new_consecutive_plus, new_jackpot_pity, was_jackpot = get_change_with_pity_and_jackpot(consecutive_plus, jackpot_pity)
    ...
    try:
        ...
        # Информация о накоплениях (только факты, без цифр)
        if was_jackpot:
...
async def fat_leaderboard(ctx):
    ...
    for i, (user_name, number, last_update, consecutive_plus, jackpot_pity) in enumerate(users, 1):
        ...
        # Добавляем информацию о накоплениях (эмодзи без цифр)
        pity_emojis = []
...
async def fat_info(ctx, member: discord.Member = None):
    ...
    # Показываем только эмодзи, без цифр
    pity_emojis = []
    ...
    embed.add_field(name="Команда", value=cooldown_status, inline=False)
...
async def cooldown_info(ctx):
    ...
    embed = discord.Embed(
        title=f"⏰ Кулдаун на сервере {ctx.guild.name}",
    ...
    if can_use:
        ...
        embed.add_field(name="Статус", value=f"⏳ Будет доступна через {format_time(remaining)}", inline=False)
